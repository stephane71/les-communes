...
const { TABLE, GLOBAL_SECONDARY_INDEXES_DEFINITION, ATTRIBUTES_DEFINITION } =
  DEFINITIONS;
const { PAY_PER_REQUEST } = BillingMode;
const { ALL } = ProjectionType;

const Properties = {
  TableName: TABLE.NAME,
  BillingMode: PAY_PER_REQUEST,
  AttributeDefinitions: Array.from(ATTRIBUTES_DEFINITION.values()).map(
    ({ name, type }) => ({
      AttributeName: name,
      AttributeType: type,
    })
  ...
  GlobalSecondaryIndexes: Array.from(
    GLOBAL_SECONDARY_INDEXES_DEFINITION.values()
  ).map(({ name, hashKey, rangeKey }) => ({
    IndexName: name,
    Projection: {
      ProjectionType: ALL,
    },
